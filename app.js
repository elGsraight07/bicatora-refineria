...
// Waits for Firebase to be ready, then boots the app

const TYPE_LABELS = {
  shutdown:'Paro de planta', repair:'Reparación', catalyst:'Catalizador',
  material:'Materiales', maintenance:'Mantenimiento', other:'Otro'
};
const TYPE_COLORS = {
  shutdown:'#E24B4A', repair:'#EF9F27', catalyst:'#1D9E75',
  material:'#185FA5', maintenance:'#7F77DD', other:'#888780'
};
const TYPE_RGB = {
  shutdown:[226,75,74], repair:[239,159,39], catalyst:[29,158,117],
  material:[55,138,221], maintenance:[127,119,221], other:[136,135,128]
};
...
let fileContents = {};  // eventId -> [{name, content}]
let fb;                 // Firebase helpers
...
function setSyncStatus(state) {
  ...
  const map = {
    ...
    offline: ['sync-offline', '<i class="ti ti-cloud-off"></i> Sin conexión'],
  };
  el.className = 'sync-badge ' + map[state][0];
...
window.switchTab = function(tab) {
  document.querySelectorAll('.nav-tab').forEach((t,i) =>
    t.classList.toggle('active', ['dashboard','events','gantt','ai'][i] === tab));
...
function renderDashboard() {
  ...
  const wg    = events.filter(e => e.gantt && e.gantt.length).length;
  const tf    = events.reduce((s,e) => s + (e.files||[]).length, 0);
  document.getElementById('stats-grid').innerHTML = `
    ...
    <div class="stat-card"><div class="stat-label">Archivos adjuntos</div><div class="stat-value" style="color:#854F0B">${tf}</div></div>
  `;
  document.getElementById('recent-list').innerHTML =
    [...events].sort((a,b) => new Date(b.date)-new Date(a.date)).slice(0,3).map(eventCardHTML).join('');
}
...
window.renderEvents = function() {
  ...
  document.getElementById('filter-bar').innerHTML = allTypes.map((t,i) =>
    `<button class="filter-btn${activeFilter===t?' active':''}" onclick="setFilter('${t}')">${labels[i]}</button>`
  ).join('');
...
window.setFilter = function(t) { activeFilter = t; renderEvents(); };
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div class="file-drop" onclick="document.getElementById('nfi').click()">
              ...
              Adjuntar archivos (PDF, Excel, Word, imágenes, TXT...)
...
window.toggleNewFields = function() {
  ...
  const el = document.getElementById('f-extra-fields');
  if (!el) return;
  if (['shutdown','repair','maintenance'].includes(t)) {
    el.innerHTML = `<div class="form-grid">
      <div class="form-group"><label>Duración estimada (días)</label><input type="number" id="f-duration" min="1" /></div>
      <div class="form-group"><label>Materiales / órdenes</label><input type="text" id="f-materials" /></div>
    </div>`;
  } else if (t === 'catalyst') {
    el.innerHTML = `<div class="form-grid">
      <div class="form-group"><label>Catalizador (tipo y cantidad)</label><input type="text" id="f-catalyst" /></div>
      <div class="form-group"><label>Materiales</label><input type="text" id="f-materials" /></div>
    </div>`;
  } else { el.innerHTML = ''; }
};
...
window.previewNewFiles = function(input) {
  ...
  document.getElementById('nfp').innerHTML = pendingFiles.map(f =>
    `<div class="file-item"><i class="ti ti-file" style="font-size:16px;color:#185FA5"></i>
     ...
     <span class="file-size">${fmtSize(f.size)}</span></div>`
  ).join('');
};
...
window.saveNewEvent = async function() {
  ...
  try {
    ...
    if (pendingFiles.length) {
      ...
      // Extract text from readable files for AI
...
async function uploadFiles(eventId, files) {
  ...
  for (const file of files) {
    ...
    uploaded.push({ name: file.name, size: fmtSize(file.size), type: file.name.split('.').pop(), url, path });
  }
  return uploaded;
}
...
function renderDetailModal(e) {
  ...
  const filesHTML = `
    <div class="file-drop" onclick="document.getElementById('fi-${e.id}').click()">
      <i class="ti ti-upload" style="font-size:20px;display:block;margin-bottom:6px"></i>
      Adjuntar archivos — se guardan en Firebase Storage
...
window.switchDetailTab = function(tab, id) {
  ...
  document.querySelectorAll('.inner-tab').forEach((t,i) =>
    t.classList.toggle('active', ['info','gantt','files'][i] === tab));
...
window.openAddTaskInDetail = function(eid) {
  const f = document.getElementById('dt-add-form');
...
window.deleteTask = async function(tid, eid) {
  ...
  if (dtc) dtc.innerHTML = buildGanttHTML(gantt, ev.date, true, eid);
  else loadGanttTab();
};
...
window.startEditTask = function(tid, eid) {
  const ev = events.find(e => e.id === eid);
  if (!ev) return;
  const t  = (ev.gantt||[]).find(g => g.id === tid);
  if (!t)  return;
  ...
  cells[2].innerHTML = `<div style="display:flex;flex-direction:column;gap:3px">
    ...
    <div style="display:flex;gap:3px;align-items:center">
      ...
      <input type="number" value="${t.dur}" id="edit-dur-${tid}" min="1" style="font-size:11px;padding:2px 4px;width:52px" />
    </div>
...
window.saveEditTask = async function(tid, eid) {
  ...
  const dtc = document.getElementById('dt-gantt-content');
  if (dtc) dtc.innerHTML = buildGanttHTML(ev.gantt, ev.date, true, eid);
  else loadGanttTab();
};
...
window.attachFilesToEvent = async function(eid, input) {
  ...
  try {
    ...
    const allFiles = [...(ev.files||[]), ...uploaded];
...
function fileItemHTML(f, eid) {
  ...
  const link = f.url ? `<a href="${f.url}" target="_blank" style="font-size:11px;color:var(--blue);display:flex;align-items:center;gap:3px"><i class="ti ti-external-link" style="font-size:11px"></i> Abrir</a>` : '';
  return `<div class="file-item">
    ...
    <button style="border:none;background:none;cursor:pointer;color:var(--text-muted);padding:2px"
      onclick="removeFile('${eid}','${f.path||''}','${f.name}')">
...
window.populateGanttSelect = function() {
  ...
  sel.innerHTML = '<option value="">— Seleccionar evento —</option>' +
    events.map(e => `<option value="${e.id}"${e.id===cur?' selected':''}>${e.title.substring(0,52)}</option>`).join('');
};
...
window.loadGanttTab = function() {
  ...
  btn.style.display = ev.gantt&&ev.gantt.length ? 'inline-flex' : 'none';
  const sd = document.getElementById('gt-start-date');
  if (sd) sd.value = ev.date;
};
...
function buildGanttHTML(tasks, eventStartDate, editMode, eventId) {
  ...
  days.forEach(d => {
    ...
    if (cur && cur.key===key) { cur.span++; }
    else { cur = {key, label:`${MONTHS_ES[d.getMonth()]} ${d.getFullYear()}`, span:1}; monthSpans.push(cur); }
  });
  const today = new Date(); today.setHours(0,0,0,0);

  let html = `<div class="gantt-wrap"><table class="gantt-table"><thead><tr>`;
  if (editMode) {
    html += `<th class="gantt-task-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>
             <th class="gantt-resp-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>
             <th class="gantt-action-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>`;
  } else {
    html += `<th class="gantt-task-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>
             <th class="gantt-resp-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>`;
  }
  ...
  html += `</tr><tr>`;
  if (editMode) {
    html += `<th class="gantt-task-col" style="background:var(--bg);font-size:11px;font-weight:500;color:var(--text-muted);text-align:left;padding:4px 8px;border-bottom:1px solid var(--border)">Actividad</th>
             <th class="gantt-resp-col" style="background:var(--bg);font-size:11px;color:var(--text-muted);border-bottom:1px solid var(--border)">Responsable</th>
             <th class="gantt-action-col" style="background:var(--bg);border-bottom:1px solid var(--border)"></th>`;
  } else {
    html += `<th class="gantt-task-col" style="background:var(--bg);font-size:11px;font-weight:500;color:var(--text-muted);text-align:left;padding:4px 8px;border-bottom:1px solid var(--border)">Actividad</th>
             <th class="gantt-resp-col" style="background:var(--bg);font-size:11px;color:var(--text-muted);border-bottom:1px solid var(--border)">Responsable</th>`;
  ...
  days.forEach(d => {
    ...
    const cls = isT ? ' today-col' : (dow===0||dow===6 ? ' weekend' : '');
    html += `<th class="gantt-hdr-day${cls}">${d.getDate()}</th>`;
  });
  html += `</tr></thead><tbody>`;

  tasks.forEach(t => {
    const tStart = parseDate(t.startDate);
    const color  = GANTT_COLORS[t.color % GANTT_COLORS.length] || '#378ADD';
    if (editMode) {
      html += `<tr class="task-row" id="task-row-${t.id}">
        <td class="gantt-task-col" style="font-size:12px">${t.name}</td>
        <td class="gantt-resp-col">${t.resp||'—'}</td>
        <td class="gantt-action-col"><div style="display:flex;gap:4px">
          <button class="btn-edit"   onclick="startEditTask('${t.id}','${eventId}')"><i class="ti ti-pencil" style="font-size:12px"></i></button>
          <button class="btn-danger" onclick="deleteTask('${t.id}','${eventId}')"><i class="ti ti-trash" style="font-size:12px"></i></button>
        </div></td>`;
    } else {
      html += `<tr class="task-row"><td class="gantt-task-col" style="font-size:12px">${t.name}</td><td class="gantt-resp-col">${t.resp||'—'}</td>`;
    }
    days.forEach(d => {
      ...
      html += `<td class="gantt-day-col">${inBar
        ? `<div style="height:18px;background:${color};opacity:0.85;border-radius:${iF?'4px':'0'} ${iL?'4px':'0'} ${iL?'4px':'0'} ${iF?'4px':'0'};margin:0 1px"></div>`
        : '&nbsp;'}</td>`;
    });
    html += '</tr>';
  });
...
function addPdfFooter(doc) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.line(10, pgH-9, pgW-10, pgH-9);
    doc.setTextColor(160,160,160); doc.setFontSize(7); doc.setFont('helvetica','normal');
    doc.text(dateStr, 10, pgH-5);
    doc.text(`${i} / ${pageCount}`, pgW-10, pgH-5, {align:'right'});
  }
}
...
window.exportGanttPDF = function(eventId) {
  ...
  monthSpans.forEach(ms=>{doc.text(ms.label,mX+2,startY+4);mX+=ms.span*dayW;});

  const dayHeaderY=startY+headerH/2;
  doc.setFillColor(242,245,248); doc.rect(startX,dayHeaderY,taskColW+respColW+totalDays*dayW,headerH/2,'F');
  doc.setFontSize(6); doc.setFont('helvetica','normal');
  days.forEach((d,i)=>{
    const dow=d.getDay();
    doc.setTextColor(dow===0||dow===6?[200,80,80]:[110,110,110]);
    doc.text(String(d.getDate()),startX+taskColW+respColW+i*dayW+dayW/2,dayHeaderY+3.5,{align:'center'});
  });
  ...
  tasks.forEach((t,ti)=>{
    ...
    days.forEach((d,i)=>{
      if(d>=tStart&&d<addDays(tStart,t.dur)){
        ...
        doc.roundedRect(bx+0.8,rowY+1.5,dayW-1.6,rowH-3,iF?1:0,iL?1:0,'F');
      }
    });
...
window.exportAllEventsPDF = function() {
  ...
  sorted.forEach((ev,idx)=>{
    ...
    const extras=[];
    if(ev.catalyst)extras.push(`Catalizador: ${ev.catalyst.substring(0,40)}`);
    if(ev.materials)extras.push(`Materiales: ${ev.materials.substring(0,50)}`);
...
window.exportEventPDF = function(eid) {
  ...
  const tl=doc.splitTextToSize(ev.title,pgW-margin*2-5);
  doc.text(tl,margin+6,17);
  ...
  fields.forEach(([label,val])=>{
    ...
    doc.text(doc.splitTextToSize(String(val),pgW-margin*2-35)[0],margin+35,y);
    y+=8;
  });
  y+=6;
  if(ev.notes){
    doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(60,60,60);
    doc.text('Notas y observaciones',margin,y); y+=5;
    doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(40,40,40);
    const nl=doc.splitTextToSize(ev.notes,pgW-margin*2);
    doc.text(nl,margin,y); y+=nl.length*5+6;
  }
  if(ev.files&&ev.files.length){
    if(y>pgH-30){doc.addPage();y=14;}
    doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(60,60,60);
    doc.text('Archivos adjuntos',margin,y); y+=5;
    ev.files.forEach(f=>{doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(40,40,40);doc.text(`• ${f.name}  (${f.size})`,margin+3,y);y+=5;});
  }
  addPdfFooter(doc);
...
// ─── AI ─────────────────────────────────────────────────────────────────────
const SUGGESTIONS = [
  '¿Cuándo fue el último paro de planta?',
  '¿Qué catalizadores se han usado?',
  '¿Quién estuvo a cargo del turnaround FCC?',
  'What materials were ordered in 2024?',
];

function renderAIChips() {
  const el = document.getElementById('ai-chips');
  if (el) el.innerHTML = SUGGESTIONS.map(s =>
    `<button class="ai-chip" onclick="askAI('${s}')">${s}</button>`
  ).join('');
}

window.sendAI = function() {
  const i = document.getElementById('ai-input');
  const q = i.value.trim(); if (!q) return; i.value = ''; askAI(q);
};
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: `You are an expert refinery operations assistant with full access to the plant's historical event log stored in Firebase. Data may be in Spanish or English — respond in the same language as the user. Be concise, technical, and cite specific data (dates, names, durations, materials) when available. You also have access to Gantt task details including task names, start dates, durations in days, and responsible persons.\n\nPLANT HISTORY FROM FIREBASE:\n${ctx}`,
        messages: [{ role: 'user', content: question }]
      })
    });
    const data = await res.json();
    chat.removeChild(thinking);
    addMsg(data.content?.[0]?.text || 'Sin respuesta.', 'ai');
  } catch(err) {
    chat.removeChild(thinking);
    addMsg('Error al conectar con el asistente de IA.', 'ai');
  }
};
